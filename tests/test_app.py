...
def test_validate_endpoint(start_server, get_bad_json_requests, get_good_json_requests):
    ...
    assert (
        response.json()["error"] == "Invalid request. Must contain json with data key."
    )

    # Fail due to validation cases and wrong json fields
    for bad_json, message in get_bad_json_requests:
        response = requests.post(
            f"{API_ENDPOINT}/validate", json={"data": [bad_json]}, timeout=1
        )
        assert response.status_code == 400
    ...
    response = requests.post(f"{API_ENDPOINT}/validate", timeout=1, json=good_json)
    assert response.status_code == 200


# Test get factors
def test_getfactors_endpoint(
    start_server, get_bad_json_requests, get_good_json_requests
):
    """Test the get_factors endpoint."""
    # Checking to make sure it attempts validation
    bad_json, message = get_bad_json_requests[2]
    response = requests.post(
        f"{API_ENDPOINT}/get_factors", timeout=1, json={"data": [bad_json]}
    )
    assert response.status_code == 400
    assert response.json()["error"] == message
    ...
    response = requests.post(f"{API_ENDPOINT}/get_factors", timeout=1, json=good_json)
    assert response.status_code == 200
